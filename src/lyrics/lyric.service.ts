...
import { CreatelyricDto, QuerylyricDto, UpdatelyricDto } from "src/dto/lyric.dto";
import { lyric, type lyricDocument } from "src/schemas/lyric.schema";


@Injectable()
export class lyricService {
  constructor(@InjectModel(lyric.name) private lyricModel: Model<lyricDocument>) {}
  ...
  async findAll(queryDto: QuerylyricDto): Promise<{ data: lyric[]; total: number }> {
  ...
  async update(id: string, updatelyricDto: UpdatelyricDto): Promise<lyric> {
    const updatedlyric = await this.lyricModel.findByIdAndUpdate(id, updatelyricDto, { new: true }).exec();
